...
@Command(name = "pmatch", playerOnly = true)
public class CommandPMatch implements ICommand{
        ...
        if(ip.getParty() != null){
            ...
            if(party.getLeader().equalsIgnoreCase(p.getName())){
                if(!Queue.inPartyQueue(party) && !Queue.inRankedPartyQueue(party)){
                    if(!party.canDuel()){
                        p.sendMessage(ChatColor.RED+"One or mode of your party members is not at spawn.");
                        return;
                    }
                    ...
                    new LadderSelect(ip){
                        ...
                        public void onSelect(Ladder ladder) {
                            Queue.addToPartyQueue(party, ladder);
